		...
		<main <?php st_text_copyck(); ?>>
			<article>
				...
				<?php else : ?>
						...
						<?php
						if ( trim( $GLOBALS['stdata9'] ) !== '' ) { // 固定記事挿入していない場合
						} else {
						?>
							...
							<?php if ( is_front_page() && is_active_sidebar( 12 ) ) { ?>
								<?php
								if ( function_exists( 'dynamic_sidebar' ) && dynamic_sidebar( 12 ) ) :
else : // トップ上部のウィジェット
?>
								<?php endif; ?>
							<?php } ?>
	...
	get_template_part( 'popular-thumbnail-on' );
				...
				<aside>
					<h2>なろう小説・書籍化作品をまとめてみました</h2>
					<p>

						カテゴリごとに小説をまとめて探しやすくしてあります。
					</p>
					...
					<?php if ( is_front_page() && is_active_sidebar( 13 ) ) { ?>
						<?php
						if ( function_exists( 'dynamic_sidebar' ) && dynamic_sidebar( 13 ) ) :
else : // トップ下部のウィジェット
?>
						<?php endif; ?>
					<?php } ?>
